var checkpass = function(node, move) {
  if (node.board[move[0]] == "P" && move[0] > 80 && move[0] < 89 && move[0] == move[1] + 20) {
...
var checkQueen = function(node, move) {
  if (node.board[move[0]] == "P" && move[1] > 20 && move[1] < 29) {
    return true;
  } else {
    return false;
  }
...
var registerPass = function(node, move) {
  var b = node.board.split("");
  if (b[move[0]] == 'P' && (move[1] == move[0] - 9 || move[1] == move[0] - 11) && b[move[1]] == ".") {
    b[move[1] + 10] = ".";
  }
  node.board = b.join("");
  return node;
